...
from antenna_controller import Position, AntennaError
...
class ObserverLocation:
    ...
    def __post_init__(self):
        ...
        if not (-90 <= self.latitude <= 90):
            raise ValueError(f"Szerokość geograficzna musi być w zakresie -90° do +90°")
        if not (-180 <= self.longitude <= 180):
            raise ValueError(f"Długość geograficzna musi być w zakresie -180° do +180°")
...
class AstronomicalPosition:
    ...
    elevation: float  # Elewacja w stopniach (-90 do +90)
...
class AstronomicalCalculator:
    ...
    def __init__(self, observer_location: ObserverLocation):
        ...
        # Cache dla gwiazd
        self._star_cache: Dict[str, ephem.FixedBody] = {}
    ...
    def get_position(self,
                     ...
                     observation_time: Optional[datetime] = None) -> AstronomicalPosition:
        """
        ...
        distance = astronomical_object.earth_distance if hasattr(astronomical_object, 'earth_distance') else 0.0
    ...
    def _get_star_by_name(self, star_name: str) -> ephem.FixedBody:
    ...
    @staticmethod
    def _create_star_from_coordinates(ra_hours: float, dec_degrees: float) -> ephem.FixedBody:
    ...
    def get_sun_position(self, observation_time: Optional[datetime] = None) -> AstronomicalPosition:
        """Skrócona metoda dla pozycji Słońca"""
        return self.get_position(AstronomicalObjectType.SUN, observation_time=observation_time)

    def get_moon_position(self, observation_time: Optional[datetime] = None) -> AstronomicalPosition:
    ...
    def get_planet_position(self, planet: AstronomicalObjectType,
                            observation_time: Optional[datetime] = None) -> AstronomicalPosition:
        """Skrócona metoda dla pozycji planet"""
        if planet not in [AstronomicalObjectType.MERCURY, AstronomicalObjectType.VENUS,
                          AstronomicalObjectType.MARS, AstronomicalObjectType.JUPITER,
                          AstronomicalObjectType.SATURN, AstronomicalObjectType.URANUS,
    ...
    def get_star_position(self, star_name: str,
                          observation_time: Optional[datetime] = None) -> AstronomicalPosition:
        """Skrócona metoda dla pozycji gwiazdy"""
        return self.get_position(AstronomicalObjectType.STAR, object_name=star_name,
                                 observation_time=observation_time)

    def get_custom_position(self, ra_hours: float, dec_degrees: float,
                            observation_time: Optional[datetime] = None) -> AstronomicalPosition:
        """Skrócona metoda dla pozycji obiektu o podanych współrzędnych"""
        return self.get_position(AstronomicalObjectType.CUSTOM,
                                 star_coordinates=(ra_hours, dec_degrees),
                                 observation_time=observation_time)

    def calculate_rise_set_times(self, object_type: AstronomicalObjectType,
                                 object_name: Optional[str] = None,
                                 star_coordinates: Optional[Tuple[float, float]] = None,
                                 date: Optional[datetime] = None) -> Dict[str, Optional[datetime]]:
        """
    ...
    def is_object_visible(self, object_type: AstronomicalObjectType,
                          object_name: Optional[str] = None,
                          star_coordinates: Optional[Tuple[float, float]] = None,
                          min_elevation: float = 0.0,
                          observation_time: Optional[datetime] = None) -> bool:
        """Sprawdza, czy obiekt jest widoczny (nad horyzontem)"""
        position = self.get_position(object_type, object_name, star_coordinates, observation_time)
...
class AstronomicalTracker:
    ...
    def create_position_function(self,
                                 object_type: AstronomicalObjectType,
                                 object_name: Optional[str] = None,
                                 star_coordinates: Optional[Tuple[float, float]] = None,
        ...
        def get_position() -> Optional[Position]:
            try:
                ...
                # Sprawdź widoczność
                if not ast_position.is_visible or ast_position.elevation < min_elevation:
                    return None
    ...
    def track_coordinates(self, ra_hours: float, dec_degrees: float, min_elevation: float = 10.0):
...
OBSERVATORIES = {
    'poznan': ObserverLocation(52.40030228321106, 16.955077591791788, 75, "Poznań Polanka")
}
...
if __name__ == "__main__":
    ...
    for obj_type, obj_name in objects_to_test:
        try:
            ...
            if antenna_pos:
                print(f"  Pozycja anteny: Az={antenna_pos.azimuth:.2f}°, El={antenna_pos.elevation:.2f}°")
            print()
    ...
    if sun_position:
        print(f"Słońce - pozycja anteny: Az={sun_position.azimuth:.2f}°, El={sun_position.elevation:.2f}°")
    else:
